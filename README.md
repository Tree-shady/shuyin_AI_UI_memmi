...
- **保持所选供应商**：系统会记住用户最后选择的供应商，直到下次手动更改，提供连续一致的使用体验
...
- **插件扩展系统**：支持通过插件机制扩展应用功能，如计算器和天气查询插件
...
- **已选供应商ID(SelectedProviderId)**：记录用户最后选择的供应商ID，确保应用重启后保持相同的供应商选择
...
A: 应用会在config.json文件中保存SelectedProviderId字段，记录您最后选择的供应商ID。下次启动应用时，系统会自动加载这个设置，确保您继续使用相同的供应商，直到您手动更改。

**Q: 如何禁用保持所选供应商功能？**

A: 目前该功能默认启用且无法直接禁用，但您可以通过手动编辑config.json文件，将SelectedProviderId字段设置为空字符串，这样下次启动应用时将使用默认供应商。
